...
class ConfigTestRunner:
    ...
    async def _test_embedding(
        ...
        probe_texts = [
            "Lumen embedding smoke test",
            "Lumen retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        run.emit("info", "Running search query: Lumen configuration health check")
        result = web_search("Lumen configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        sample = "Lumen voice check. 这是一段语音合成测试。"
