...
def parse_language(language: Any) -> str:
    """Resolve a language value to a normalized code (default ``"zh"``).

    Maps the spelled-out aliases of the two shipped locales
    (``"english"``/``"en"``, ``"chinese"``/``"cn"``/``"zh"``) and otherwise
    passes the code through ``normalize_language`` unchanged (e.g. Japanese
    stays ``"ja"`` rather than collapsing to Chinese).
    """
