...
class RAGService:
    ...
    Lumen's authoritative KB config, with metadata as a legacy fallback.
    """
    ...
    def _resolve_provider(self, kb_name: Optional[str]) -> str:
        """Pick the provider for ``kb_name`` from Lumen's binding."""
    ...
    def _capture_raw_logs(self, event_sink):
        ...
        class _ProcessLogEvent:
            ...
            @classmethod
            def from_record(cls, record: logging.LogRecord) -> "_ProcessLogEvent":
                return cls(
        ...
        class _ContextFilter(logging.Filter):
            def filter(self, record: logging.LogRecord) -> bool:
                record.log_context = dict(getattr(record, "log_context", {}) or {})
                return True
        ...
        def should_skip_noisy_retrieve_log(event: _ProcessLogEvent) -> bool:
            if event.level != "INFO":
                return False
            ...
            if logger_name == "nano-vectordb" and (
                ...
                return True
            if (
                logger_name.startswith("lumen.shared._util.embedding.")
                and (
        ...
        class _NamedRawLogHandler(logging.Handler):
            ...
            def emit(self, record: logging.LogRecord) -> None:
                try:
                    result = emit(_ProcessLogEvent.from_record(record))
