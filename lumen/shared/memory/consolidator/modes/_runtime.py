...
async def write_doc_checkpoint(
    ...
    *,
    ...
    from lumen.shared.memory.consolidator.runs import push_undo_checkpoint
...
def _strip_thinking_delta(delta: str, in_block: bool) -> tuple[str, bool]:
    """Remove streamed thinking blocks before they reach the workbench UI."""
    out: list[str] = []
    text = delta
    while text:
        lower = text.lower()
        if in_block:
            close_at = lower.find(" response")
            close_len = len(" response")
