"""Branded banner + localized labels for ``lumen start`` / ``lumen init``.
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": f"Run `lumen start` to launch {PRODUCT_NAME}.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `lumen start` again."
        ),
        ...
        "start.port_in_use": (
            f"{PRODUCT_NAME} cannot start because port(s) already in use: {{ports}}. "
    ...
    "zh": {
        ...
        "init.next_step": f"运行 `lumen start` 启动 {PRODUCT_NAME}。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `lumen start`。"
        ),
        ...
        "start.port_in_use": (
            f"无法启动 {PRODUCT_NAME},端口已被占用: {{ports}}。"
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title=f"[bold bright_cyan]{PRODUCT_NAME}[/]",
