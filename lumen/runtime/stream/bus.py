    ...
    async def tool_call(
        ...
        await self.emit(
            StreamEvent(
                ...
                stage=stage,
                content=tool_name,
                metadata=_merge_trace_metadata({"args": args}, metadata),
            )
        )
    ...
    async def tool_result(
        ...
        await self.emit(
            StreamEvent(
                ...
                stage=stage,
                content=result,
                metadata=_merge_trace_metadata({"tool": tool_name}, metadata),
            )
        )
    ...
    async def sources(
        ...
        await self.emit(
            StreamEvent(
                ...
                source=source,
                stage=stage,
                metadata=_merge_trace_metadata({"sources": sources}, metadata),
            )
        )
    ...
    async def result(
        ...
        await self.emit(
            StreamEvent(
                ...
                source=source,
                metadata=_merge_trace_metadata(data, metadata),
            )
        )
