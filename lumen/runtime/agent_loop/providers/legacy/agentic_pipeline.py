...
from lumen.runtime.agent_loop.engine.tool_dispatch import (
    MAX_PARALLEL_TOOL_CALLS,
    DispatchOutcome,
    dispatch_tool_calls,
)
from lumen.runtime.agent_loop.engine.usage import UsageTracker
...
from lumen.runtime.tool_protocol import ToolLookup
...
from lumen.shared._util.config import get_chat_params
from lumen.shared._util.knowledge import KbManifest, render_manifest_note
...
class AgenticChatPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        # Loop capabilities active for this turn, contributed by the owning
        # mode (e.g. mode.learn's ``MasteryLoopCapability``) through the
        # injected ``runtime.agent_loop`` contract. The runtime itself never
        # imports ``lumen.modes`` (Architecture Gate
        # ``test_runtime_does_not_import_modes``), so a generic chat turn has
        # no capabilities and this stays empty.
        self._loop_capabilities: tuple[LoopCapability, ...] = loop_capabilities or ()
    ...
    async def _exec_allowed(self, context: UnifiedContext) -> bool:
        try:
            from lumen.shared._util.sandbox import IsolationLevel, get_sandbox_service
    ...
    def _active_loop_capabilities(self, context: UnifiedContext) -> tuple[LoopCapability, ...]:
        return tuple(
            cap for cap in getattr(self, "_loop_capabilities", ()) if cap.is_active(context)
        )
    ...
    def _augment_tool_kwargs(
        ...
        if tool_name == "rag":
            ...
            from lumen.shared._util.sandbox import Mount
